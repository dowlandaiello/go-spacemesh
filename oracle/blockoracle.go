...
func NewMinerBlockOracle(committeeSize int32, layersPerEpoch uint16, activationDb ActivationDb,
	beaconProvider *EpochBeaconProvider, vrfSigner *crypto.VRFSigner) *MinerBlockOracle {
...
func (bo *MinerBlockOracle) BlockEligible(layerID mesh.LayerID, nodeID mesh.NodeId) ([]mesh.BlockEligibilityProof, error) {

	epochNumber := uint64(layerID) / uint64(bo.layersPerEpoch)

	if bo.proofsEpoch != epochNumber {
		err := bo.calcEligibilityProofs(epochNumber, nodeID)
		if err != nil {
			return nil, err
		}
	}
...
func (bo *MinerBlockOracle) calcEligibilityProofs(epochNumber uint64, nodeID mesh.NodeId) error {
	epochBeacon := bo.beaconProvider.GetBeacon(epochNumber)

	atxIDs, err := bo.activationDb.GetNodeAtxIds(nodeID)
	if err != nil {
