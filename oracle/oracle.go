...
type BlockOracle interface {
	BlockEligible(layerID mesh.LayerID, nodeID mesh.NodeId) ([]mesh.BlockEligibilityProof, error)
}
...
func (bo *localBlockOracle) BlockEligible(layerID mesh.LayerID, nodeID mesh.NodeId) ([]mesh.BlockEligibilityProof, error) {
	eligible := bo.oc.Eligible(uint32(layerID), bo.committeeSize, nodeID.Key, nil)
...
// Eligible checks whether we're eligible to mine a block in layer i
func (bo *blockOracle) BlockEligible(layerID mesh.LayerID, nodeID mesh.NodeId) ([]mesh.BlockEligibilityProof, error) {
	eligible := bo.oc.Eligible(uint32(layerID), bo.committeeSize, nodeID.Key)
	var proofs []mesh.BlockEligibilityProof
	if eligible {
		proofs = []mesh.BlockEligibilityProof{}
	}
